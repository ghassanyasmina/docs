...
## Introduction
This guide describes Yasmina’s Motor Insurance APIs for Multiple Insurance Providers (for single insurance provider, [click here](/single-provider-integrations/car)). These APIs cover all essential operations for Comprehensive motor insurance — including requesting quotes, issuing and managing policies, generating payment links, and handling claims.
Our APIs follow RESTful principles and are designed with consistent input and output structures, making them easy to integrate, test, and maintain within your systems.
...
Quotes are the different premiums, benefits, and deductibles offered by insurance providers. Before issuing a Comprehensive Motor insurance policies you need to give the car details to get different quotes from insurance providers. These quotes are then used to issuing the policy.
